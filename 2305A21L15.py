...
def Gen_Eff(V, CL, IL, K, Rse, Ra):
    ...
    CUL = ((K * IL)**2)*(Rse + Ra)  # Copper losses
    Pin = Pout + (CL*1000) + CUL  # Input power
    Eff = (Pout / Pin) * 100  # Efficiency
    return Eff, CUL

# Streamlit app
st.title("2305A21L15 - P10")
...
CL = st.number_input("CL:in KiloWatts", value=50.0, step=1.0)
